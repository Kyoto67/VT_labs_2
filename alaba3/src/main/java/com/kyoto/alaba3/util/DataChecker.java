...
public class DataChecker {
    ...
    private boolean hitCheck(double x, double y, double r) {
        ...
        if (normalizedY>0) {
            return (normalizedY <= 0.5 && normalizedX <=0 && normalizedX >= -1 );
        } else if(normalizedY<0) {
            return ( (normalizedX >= 0 && normalizedX <= Math.sqrt(1 - normalizedY * normalizedY) || ( normalizedY >= -1 && normalizedX <= 0 && normalizedX >= (-normalizedY - 1) / 2 ) ) );
        } else {
            return (normalizedX >= -1 && normalizedX <= 1);
        }
